...
def download_document(url):
    try:
        ...
        # Debugging: Print response text to check structure
    ...
    if link:
        ...
        if document_url:
            ...
            if response.status_code == 200:
                ...
                save_path = filedialog.asksaveasfilename(defaultextension=".pdf", initialfile=file_name)
                
                if save_path:
                    with open(save_path, "wb") as document_file:
                        document_file.write(document_content)
                    messagebox.showinfo("Success", "Document downloaded and saved successfully")
                else:
                    messagebox.showwarning("Save Error", "Save operation cancelled")
            else:
                messagebox.showerror("Error", "Failed to download document")
        else:
            messagebox.showerror("Error", "Invalid document URL")
    else:
        messagebox.showwarning("Input Error", "Please enter a link")

app = tk.Tk()
app.title("Document Downloader")

tk.Label(app, text="Enter the Issuu document link:").pack(pady=10)
entry_link = tk.Entry(app, width=50)
entry_link.pack(pady=10)

tk.Button(app, text="Download Document", command=download_and_save_document).pack(pady=10)

# Add Browse button to select save directory
tk.Button(app, text="Browse", command=lambda: filedialog.askdirectory()).pack(pady=10)
